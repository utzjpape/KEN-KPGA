...
merge m:1 tsector using "${gsdDataRaw}/1-Intro/sector_agg_growth.dta", nogen	
...
*CHECK: Assume "length" is when child is measured lying down (f23==2) and "height" is when child is measured standing (f23==1)
	...
	* 	if age missing & indicator missing, use length of child to figure.
	...
	merge __000001 _agedays using "${gsdDataRaw}/1-Intro/lenanthro.dta"
...
egen uhhid=concat(id_clust id_hh)
keep uhhid b05b b_id
tempfile birth_month
save `birth_month'.dta ,replace
...
merge 1:1 uhhid b_id using `birth_month'.dta , keep(match) nogen keepusing(b05b)
