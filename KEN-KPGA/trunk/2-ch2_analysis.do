...
foreach var in 2005 2015  {
	forvalues i = 1 / 8{
		ineqdeco $cons if kihbs == `var' & province==`i'  [aw = wta_pop]
		matrix prov_`i'_`var' = [r(p90p10), r(p75p25), r(gini)]	
	}
}	
...
foreach s in total rural urban  {
        line schange`i' x, lcolor(navy) lpattern(solid) yline(`mean_change`i'') subtitle("Growth incidence, `s'") xtitle("Share of population ranked , percent", size(small)) xlabel(, labsize(small)) ytitle("% change cons per capita", size(small)) ylabel(, angle(horizontal) labsize(small)) name(gic`i', replace)
		local i = `i' + 1
}
...
foreach s of local provinces  {
        line schange`i' x, lcolor(navy) lpattern(solid) yline(`mean_change_prov`k'') subtitle("Growth incidence, `s'")         xtitle("Share of population ranked , percent", size(small)) xlabel(, labsize(small)) ytitle("% change cons per capita", size(small)) ylabel(, angle(horizontal) labsize(small)) name(gic`i', replace)
		local i = `i' + 1
}
graph combine gic1 gic2 gic3, iscale(*0.9) title("2005-2015")
